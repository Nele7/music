      ...
      <div class="dialog" @click="cancelDefault">
        <div style="padding:20px" v-if="!isSelect">
          <div class="search">
            <p>热门搜索</p>
            <ul>
              <li v-for="(item,index) in hotSearch" :key="index" @click="changeSearch(item.first)">
                <a href="#">{{item.first}}</a>
              </li>
            </ul>
          </div>
        ...
        <div class="search-suggest-wrapper">
          <div class="suggest-box" v-for="(order,index) in suggestSearch.order" :key="index">
            ...
            <ul class="suggest-list">
              <li v-for="(item,index) in suggestSearch[order]" :key="index"><a href="">{{item.name}}</a></li>
            </ul>
          </div>
        </div>
        </el-scrollbar>
...
export default {
  ...
  methods: {
    ...
    enterSearch() {
      // 保存本地
      let index = this.historySearch.findIndex(item => item === this.valueSearch)
      if (index > -1) {
        this.historySearch.splice(index, 1)
      }
      this.historySearch.unshift(this.valueSearch)
